    ...
    try {
      ...
      for (const igdbGame of igdbResults) {
        const result: GameSearchResult = {
          ...
          title: igdbGame.name,
          platform: this.mapIGDBPlatform(igdbGame.platforms?.[0]?.abbreviation || '') as any,
        ...
        for (const rawgGame of rawgResults) {
          ...
          if (!isDuplicate) {
            const result: GameSearchResult = {
              ...
              title: rawgGame.name,
              platform: RAWGService.mapPlatform(rawgGame.platforms) as any,
  ...
  private levenshteinDistance(str1: string, str2: string): number {
    ...
    for (let j = 1; j <= str2.length; j++) {
      for (let i = 1; i <= str1.length; i++) {
        ...
        if (matrix[j] && matrix[j][i - 1] !== undefined && 
            matrix[j - 1] && matrix[j - 1][i] !== undefined && 
